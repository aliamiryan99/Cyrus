...
def init_outputs(profits, positions, simulation, balance_history, equity_history, margin_history, backtest_years, s_d,
    ...
    if average_aum == 0:
        ...
        annualized_turnover = round((total_profit / balance_history[0][2]) ** (1/float(backtest_years)), 3)
...
def index_date(data, date):
    ...
    if date < data.iloc[start]['GMT'] or date > data.iloc[end]['GMT']:
        return -1
    i = 0
    while start <= end:
        i = int((start + end) / 2)
        if data.iloc[i]['GMT'] == date:
            return i
        elif data.iloc[i]['GMT'] < date:
            start = i + 1
        elif data.iloc[i]['GMT'] > date:
            end = i - 1
    if data.iloc[i]['GMT'] <= date:
...
def index_date_v2(data, date):
    ...
    if date < data[start]['GMT'] or date > data[end]['GMT']:
        return -1
    i = 0
    while start <= end:
        i = int((start + end) / 2)
