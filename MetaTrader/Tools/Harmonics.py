...
class Harmonics:
    ...
    def __init__(self, data, symbol, extremum_window, time_range, price_range_alpha, harmonic_name, alpha, beta,
                 fibo_tolerance, \
                 pattern_direction, harmonic_list, save_result, candle_time_frame, neo_time_frame, extremum_mode,
                 is_save_statistical_results):
    ...
    def get_local_extremum_from_elliott(self):
        ...
        In_impulse_prediction_list_w5, In_zigzag_flat_prediction_list = elliott.calculate(pd.DataFrame(self.data), False, False, False, False, price_type="neo", candle_timeframe=self.candle_time_frame, neo_timeframe=self.neo_time_frame)
            ...
            for name in self.result.keys():
                if 'bullish' in self.result[name].keys():
                    ...
                    for result in self.result[name]['bullish']:
                        ...
                        sl_val = \
                        np.random.randint(low=result[9] - np.abs(result[8] - result[7]), high=result[9] + 1, size=(1))[
                            0]
                        ...
                        for j in range(0, len(tp_val)-1):
                            ...
                            STR = "ACC" + str(j + 1) + ":" + "%.1f" %Acc[j] + " _TP" + str(j + 1) + ": " + "%.3f" %tp_val[j]
