...
class SupplyAndDemand(RealTimeTool):
    ...
    def on_data(self, candle):
        ...
        self.update_fresh_sources(self.fresh_demand_sources, candle)
        self.update_fresh_sources(self.fresh_supply_sources, candle)

        self.find_movements(self.data, self.atr_window)
        self.find_new_source(self.data)

        self.redraw_sources(self.fresh_demand_sources, self.demand_color)
        self.redraw_sources(self.fresh_supply_sources, self.supply_color)

        self.check_new_candlestick()

        self.redraw_extremums()

        self.data.append(candle)

    def check_new_candlestick(self):
    ...
    def check_touch_area(self, sources, type, price):
        copy_sources = copy.copy(sources)
        for i in range(len(copy_sources)):
            if type == 1:   # Demand
                if price < copy_sources[i]['UpPrice']:
                    self.chart_tool.set_speed(0.01)
                    sl = (price - copy_sources[i]['DownPrice']) * 10 ** Config.symbols_pip[self.symbol]
                    ...
                    self.chart_tool.buy(self.symbol, v/2, sl*2, sl)
                    sources.remove(copy_sources[i])
                    self.chart_tool.set_speed(self.speed)
            if type == 2:   # Supply
                if price > copy_sources[i]['DownPrice']:
                    self.chart_tool.set_speed(0.01)
                    sl = (copy_sources[i]['UpPrice'] - price) * 10 ** Config.symbols_pip[self.symbol]
                    ...
                    self.chart_tool.sell(self.symbol, v/2, sl*2, sl)
                    sources.remove(copy_sources[i])
                    self.chart_tool.set_speed(self.speed)
