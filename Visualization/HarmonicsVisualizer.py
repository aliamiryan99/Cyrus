...
class HarmonicVisualizer:
    ...
    def __init__(self, symbol, time_frame, data, extremum_window, time_range, price_range_alpha, harmonic_name):
        ...
        self.local_min_price_area, self.local_max_price_area = get_local_extremum_area(data, self.local_min_price, self.local_max_price, self.time_range, self.price_range)

        self.bullish_result = harmonic_pattern(self.open, self.high, self.low, self.close, self.top, self.bottom, self.middle, self.local_min_price_area, self.local_max_price_area, self.symbol, self.time_frame, self.harmonic_name, "Bullish")
        self.bearish_result = harmonic_pattern(self.open, self.high, self.low, self.close, self.top, self.bottom, self.middle, self.local_min_price_area, self.local_max_price_area, self.symbol, self.time_frame, self.harmonic_name, "Bearish")

        self.bullish_result = filter_results(self.high, self.low, self.bullish_result, self.middle, self.harmonic_name, "Bullish")
    ...
    def draw(self, fig, height):
        ...
        for result in self.bullish_result:
            fig.patch([result[0], result[1], result[2]], [result[5], result[6], result[7]], alpha=0.05, color="blue")
            fig.patch([result[2], result[3], result[4]], [result[7], result[8], result[9]], alpha=0.05, color="blue")

        for result in self.bearish_result:
            fig.patch([result[0], result[1], result[2]], [result[5], result[6], result[7]], alpha=0.05, color="red")
            fig.patch([result[2], result[3], result[4]], [result[7], result[8], result[9]], alpha=0.05, color="red")
