...
class SupportResistanceVisualizer:
    ...
    def __init__(self, data, extremum_window, num_section):
        self.data = data
        ...
        local_min_value, local_max_value = self.low[self.local_min_price], self.high[self.local_max_price]
        ...
        for r_s in self.r_s_matrix:
            ...
            if not self.sections_filed[section]:
                if value > self.data[-1]['Close']:
                    fig.line(x=[0, len(self.data)], y=[value, value], line_color='#1243b1', line_width=2)
                else:
                    fig.line(x=[0, len(self.data)], y=[value, value], line_color='#f24341', line_width=2)
