...
class MEAlgorithm:
    ...
    def __init__(self, symbol, data_history, window_size, extremum_window, extremum_mode, extremum_level, extremum_pivot):
        # Open, High, Low, Close
        self.symbol = symbol
    ...
    def check_satisfy(self):
        ...
        for i in range(len(self.local_max_price)-self.extremum_level, len(self.local_max_price)):
            if self.local_max_price[i-1] < self.local_max_price[i]:
                self.buy_satisfaction = False
                break
        ...
        for i in range(len(self.local_min_price) - self.extremum_level, len(self.local_min_price)):
            if self.local_min_price[i - 1] > self.local_min_price[i]:
                self.sell_satisfaction = False
                break
