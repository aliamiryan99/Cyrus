...
class ShadowConfirmation:
    ...
    def on_tick(self):
        if self.signal_trigger == 1 and not self.candle_buy_submitted:
        ...
        elif self.signal_trigger == -1 and not self.candle_sell_submitted:
            limit_price = 0
            if self.shadow_confirmation_mode == 1:
                limit_price = max(self.data_window[-2]['Open'], self.data_window[-2]['Close'])
            elif self.shadow_confirmation_mode == 2:
    ...
    def on_data(self, candle, cash):
        self.candle_sell_submitted, self.candle_buy_submitted = False, False
