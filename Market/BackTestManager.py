...
class MarketManager:
    ...
    def __init__(self):
        ...
        self.algorithm_data, self.trailing_data, self.history_size, self.algorithms, self.re_entrance_algorithms,\
            ...
            self.account_managements, self.simulation, self.last_ticket, self.last_algorithm_signal_ticket,\
    ...
    def run(self):
        ...
        for ii in tqdm(range(back_test_length)):
            ...
            for symbol in self.symbols:
                ...
                # Account Management
                volume = self.account_managements[symbol].calculate(self.simulation.balance, symbol, price, stop_loss)
                ...
                if self.last_buy_closed[symbol] is None:
                    self.last_buy_closed[symbol] = self.simulation.get_last_buy_closed(symbol)
                if self.last_sell_closed[symbol] is None:
                    self.last_sell_closed[symbol] = self.simulation.get_last_sell_closed(symbol)
        ...
        for i in range(len(symbols)):
            ...
            algo_data = \
                algorithm_data[i].to_dict('Records')[algorithm_start_indexes[symbol] -
                                                     MarketConfig.history_size:algorithm_start_indexes[symbol]]
            configs[symbol] = MarketConfig(symbol, algo_data, MarketConfig.algorithm_name,
    ...
    def initialize_algorithms(self):
        ...
        return algorithm_data, trailing_data, history_size, algorithms, re_entrance_algorithms, recovery_algorithms,\
            close_modes, tp_sl_tools, trailing_tools, account_managements, simulation, last_ticket,\
