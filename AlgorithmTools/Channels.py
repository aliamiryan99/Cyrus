...
def get_channels(data, extremum_start_window, extremum_end_window, extremum_window_step, extremum_mode, check_window, alpha):
    ...
    for window in range(extremum_start_window, extremum_end_window, extremum_window_step):
        ...
        while min_pointer < len(local_min) and max_pointer < len(local_max):
            up_channel = check_up_channel(price_up, price_down, local_min, local_max, min_pointer, max_pointer, check_window, alpha)
            down_channel = check_down_channel(price_up, price_down, local_min, local_max, min_pointer, max_pointer, check_window, alpha)
            # Save Channels
...
def check_up_channel(price_up, price_down, local_min, local_max, local_min_pointer, local_max_pointer, check_window, alpha):
    if check_ascending(price_up, local_max[local_max_pointer-check_window+1:local_max_pointer+1]):
        if check_ascending(price_down, local_min[local_min_pointer-check_window+1:local_min_pointer+1]):
            ...
            if abs((up_slop - down_slop)/(up_slop + down_slop)) < alpha:
                return {'UpLine': up_line, 'DownLine': down_line}
    return None


def check_down_channel(price_up, price_down, local_min, local_max, local_min_pointer, local_max_pointer, check_window, alpha):
