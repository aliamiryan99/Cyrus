...
def get_parallel_channels(data, extremum_start_window, extremum_end_window, extremum_window_step, extremum_mode, check_window, alpha, beta):
    ...
    for window in range(extremum_start_window, extremum_end_window+1, extremum_window_step):
        ...
        while min_pointer < len(local_min) and max_pointer < len(local_max):
            channel = check_channels(price_up, price_down, local_min, local_max, min_pointer, max_pointer, check_window, alpha, beta, window)
            # Save Channels
...
def check_channels(price_up, price_down, local_min, local_max, local_min_pointer, local_max_pointer, check_window, alpha, beta, window):
    for max_i in local_max[local_max_pointer-check_window+1:local_max_pointer]:
        ...
        for min_i in local_min[local_min_pointer-check_window+1:local_min_pointer]:
            ...
            if is_parallel(up_line[0], down_line[0], alpha, beta):
...
def is_parallel(up_slope, down_slope, alpha, beta):
    return beta < abs((up_slope - down_slope)/(up_slope + down_slope)) < alpha
